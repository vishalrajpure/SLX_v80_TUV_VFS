<%@ Control Language="C#" ClassName="CallsActivities_PrintView" Inherits="Sage.Platform.WebPortal.SmartParts.EntityBoundSmartPartInfoProvider" %>
...
<SalesLogix:ScriptResourceProvider ID="CallsActivities_PrintViewgrdTicketActivities_Strings" runat="server">
    <Keys>
...
<div style="display:none;">
  <asp:TextBox runat="server" ID="CallsActivities_PrintViewgrdTicketActivities_DataCarrier" class="CallsActivities_PrintViewgrdTicketActivities_DataCarrier" ></asp:TextBox>
</div>
  <div id="CallsActivities_PrintViewgrdTicketActivities_Container" style="width:100%;height:100%;" class="">
    <div id="CallsActivities_PrintViewgrdTicketActivities_Grid" style="width:100%;height:100%;"
></div>
</div>
 ...
 <SalesLogix:SmartPartToolsContainer runat="server" ID="CallsActivities_PrintView_RTools" ToolbarLocation="right">
   
  <SalesLogix:PageLink ID="btnEditForm" CssClass="adminEditFormButton" runat="server" LinkType="RelativePath" ToolTip="<%$ resources: Portal, EditForm_ToolTip %>" NavigateUrl="~/FormManager.aspx?entityid=CallsActivities_PrintView&modeid=Detail" ImageUrl="~/ImageResource.axd?scope=global&type=Global_Images&key=form_manager_16x16"></SalesLogix:PageLink>
 
 
 
 
 <SalesLogix:PageLink ID="lnkCallsActivities_PrintViewHelp" runat="server" LinkType="HelpFileName" ToolTip="<%$ resources: Portal, Help_ToolTip %>" Target="MCWebHelp" NavigateUrl="CallsActivities" ImageUrl="~/ImageResource.axd?scope=global&type=Global_Images&key=Help_16x16"></SalesLogix:PageLink>
...
ScriptManager.RegisterStartupScript(Page, GetType(), "cleanupcontainer", "jQuery(\".controlslist > div:empty\").remove();", true);
 ...
 if (entity != null) 
      ...
      { 
          ...
          CallsActivities_PrintViewgrdTicketActivities_DataCarrier.Text = text; 
...
dtsTicketActivityDataSource.Bind();
                                ...
                                script.AppendLine("require(['" + Page.ResolveUrl("~/SmartParts/Ticket/CallsActivities_PrintView.js") + "'], function () {");
                                if (Page.IsPostBack)
                                {
                                    script.Append(" Sage.UI.Forms.CallsActivities.init({workspace: '" + getMyWorkspace() + "'} );");
                                }
                                else 
                                {
                                    script.AppendLine("require(['dojo/ready'], function(ready) {");
                                    script.Append("ready(function () {Sage.UI.Forms.CallsActivities.init({workspace: '" + getMyWorkspace() + "'} ); });");
                                    script.AppendLine("});");//end ready require
                                }

                                script.AppendLine("});");// end require
                                ScriptManager.RegisterStartupScript(this, GetType(), "initialize_CallsActivities", script.ToString(), true);
...
{
    get { return _formAdapter ?? (_formAdapter = new CallsActivitiesAdapter(this)); }
}

public class CallsActivitiesAdapter : Sage.Platform.WebPortal.Adapters.EntityFormAdapter, Sage.Form.Interfaces.ICallsActivities
{
    public CallsActivitiesAdapter(Sage.Platform.WebPortal.SmartParts.EntityBoundSmartPartInfoProvider smartPart)
        : base(smartPart) {}
