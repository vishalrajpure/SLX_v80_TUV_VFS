...
public partial class GeneralSearchOptionsPage : UserControl, ISmartPartInfoProvider
{
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        txtFaxProvider.Text = _options.FaxProvider;
        ...
        txtFaxProvider.Attributes.Add("style", "display:none");
...
function showSDIIsInstalledMsg() {
    ...
    private List<ListItem> GetUserSyncGroups()
    {
        ...
        var _userService = ApplicationContext.Current.Services.Get<Sage.Platform.Security.IUserService>() as SLXUserService;
        if (_userService != null)
            _userID = _userService.UserId;

        var _format = (string)GetLocalResourceObject("adhoc_group_format");


        // Load up all of the adhoc groups
        var _dataService = ApplicationContext.Current.Services.Get<IDataService>();
        ...
        FROM PLUGIN A1 WHERE (A1.TYPE = 8) AND (A1.PLUGINID IN (
          SELECT A2.PLUGINID
          FROM PLUGIN A2
          WHERE (A2.PLUGINID = A2.DATACODE))) AND (A1.USERID = '{0}') AND (UPPER(A1.FAMILY) = 'CONTACT')
        ORDER BY A1.NAME";
                ...
                {
                    if (_format != null)
                        results.Add(new ListItem(string.Format(_format, row[1], row[2]), (string)row[0]));
                }
            }
        }
    ...
    protected void _save_Click(object sender, EventArgs e)
    {
        ...
        _options.FaxProvider = FaxProviderSelectedValue.Value != "undefined" ? FaxProviderSelectedValue.Value : txtFaxProvider.Text;
