...
function (dateLocale, cell, utility, dstring, nlsStrings, declare) {
    ...
    return Sage.Utility.Activity = {
        ...
        formatActivityStartDateForCalendar: function (startDate, dateFormat) {
            if (!startDate) {
                return '';
            }
            if (Sage.Utility.Convert.isDateString(startDate)) {
            ...
            var timelessDate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate(), startDate.getHours(), startDate.getMinutes());
            if (dateFormat)
                return dateLocale.format(timelessDate, { selector: 'date', datePattern: dateFormat });
            else
                return dateLocale.format(timelessDate, { selector: 'date', datePattern: "MM/d/yy H:mm" });
        },
        ...
        formatTimelessEndDate: function (dateValue, mode, increment) {
            ...
            var pad = function (n) { return n < 10 ? '0' + n : n; };
            var utcYear = dateValue.getUTCFullYear();
            var utcMonth = dateValue.getUTCMonth() + 1;
